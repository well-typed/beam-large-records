...
largeRecord defaultPureScript [d|
      data ShoppingCart3Db (f :: * -> *) = ShoppingCart3Db {
...
test_SQL = runInMemory $ \conn -> do
      ...
      [bettyShippingInfo] <-
        runInsertReturningList $
          insertReturning (shoppingCart3Db ^. xshoppingCart3ShippingInfos) $
            insertExpressions [
                [lr| ShippingInfo |] default_ (val_ USPS) (val_ "12345790ABCDEFGHI")
              ]
    ...
    [jamesOrder1, bettyOrder1, jamesOrder2] <-
      runInsertReturningList $
        insertReturning (shoppingCart3Db ^. xshoppingCart3Orders) $
          insertExpressions [
              [lr| Order |] default_ currentTimestamp_ (val_ (pk james)) (val_ (pk jamesAddress1)) nothing_
            , [lr| Order |] default_ currentTimestamp_ (val_ (pk betty)) (val_ (pk bettyAddress1)) (just_ (val_ (pk bettyShippingInfo)))
            , [lr| Order |] default_ currentTimestamp_ (val_ (pk james)) (val_ (pk jamesAddress1)) nothing_
            ]
    ...
    let lineItems :: [LineItem]
        lineItems = [
            [lr| LineItem |] (pk jamesOrder1) (pk redBall)        10
          , [lr| LineItem |] (pk jamesOrder1) (pk mathTextbook)   1
          , [lr| LineItem |] (pk jamesOrder1) (pk introToHaskell) 4
          , [lr| LineItem |] (pk bettyOrder1) (pk mathTextbook)   3
          , [lr| LineItem |] (pk bettyOrder1) (pk introToHaskell) 3
          , [lr| LineItem |] (pk jamesOrder2) (pk mathTextbook)   1
          ]
    ...
    shippingInformationByUser <-
      runSelectReturningList $
        select $ aggregate_ (\(user, order) ->
                   let ShippingInfoId shippingInfoId = order.orderShippingInfo
                   in ( group_ user
    ...
    shippingInformationByUser' <-
      runSelectReturningList $
        select $ do
          ...
          (forUserEmail, unshippedCount) <-
            aggregate_ (\(userEmail, _order) -> (group_ userEmail, countAll_)) $
          ...
          guard_ (forUserEmail `references_` forUser)
          ...
          guard_ (userEmail `references_` forUser)
  ...
  where
    ...
    users@[james, betty, sam] = [
          [lr| User |] "james@example.com" "James" "Smith"  "b4cc344d25a2efe540adbf2678e2304c"
        , [lr| User |] "betty@example.com" "Betty" "Jones"  "82b054bd83ffad9b6cf8bdb98ce3cc2f"
        , [lr| User |] "sam@example.com"   "Sam"   "Taylor" "332532dcfaa1cbf61e2a266bd723612c"
        ]
    ...
    products = [
          [lr| Product |] default_ (val_ "Red Ball")         (val_ "A bright red, very spherical ball")                      (val_ 1000)
        , [lr| Product |] default_ (val_ "Math Textbook")    (val_ "Contains a lot of important math theorems and formulae") (val_ 2500)
        , [lr| Product |] default_ (val_ "Intro to Haskell") (val_ "Learn the best programming language in the world")       (val_ 3000)
        , [lr| Product |] default_ (val_ "Suitcase")               "A hard durable suitcase"                                       15000
        ]
    ...
    info1 :: ShippingInfo
    info1 = [lr| ShippingInfo |] 1 USPS "12345790ABCDEFGHI"
